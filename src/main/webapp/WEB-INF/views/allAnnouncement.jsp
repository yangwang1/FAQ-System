		...
		<table class="table table-bordered table-striped table-hover">
		 <caption>公告列表</caption>
			...
			<c:forEach items="${accouncePage.content}" var="accounce">
				<tr>
                    <td>${accounce.title}</td>
                    <td><a href = "${pageContext.request.contextPath}/announce/get/${accounce.id}" class="btn btn-info btn-sm" role="button">查看内容</a></td>
                    <% if(request.isUserInRole("ROLE_ADMIN")||request.isUserInRole("ROLE_TEACHER")){ %>
                    <td><a href = "${pageContext.request.contextPath}/announce/delete/${accounce.id}" class="delete btn btn-danger btn-sm" role="button">删除</a>
                    <%   }  %>
                    </td>
				</tr>
			</c:forEach>
